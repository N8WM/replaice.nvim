...
M.system = table.concat({
  ...
  "Return only the exact replacement text for REPLAICE_SELECTION.",
...
local function history_text(history, include_current)
  ...
  for index = 1, last do
    ...
    if attempt.user_feedback then
      table.insert(parts, ("<REPLAICE_USER_FEEDBACK number=\"%d\">\n%s\n</REPLAICE_USER_FEEDBACK>"):format(
        index,
...
function M.rewrite(context, request, history)
  ...
  if #history > 0 then
    table.insert(parts, "Produce a new replacement that addresses all prior feedback and does not regress on earlier fixes.")
  end
  return table.concat(parts, "\n")
end
